...
public class GitHubAccessTokenDtoSerializator implements Serializator {
  private final Gson gson = new GsonBuilder().registerTypeAdapterFactory(SerializatorTypeAdapterFactory.create()).create();

  @Override public @Nullable <T> String to(@NonNull T output) {
    return gson.toJson(output);
  }

  @SuppressWarnings("unchecked") @Override public <T> T from(@NonNull String input) {
    return (T) gson.fromJson(input, GitHubAccessTokenDto.class);
  }
}
